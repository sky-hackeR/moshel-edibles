...
<div class="container mx-auto px-6 py-12">
    ...
    <div class="text-center max-w-2xl mx-auto mb-16">
        <span class="text-mosh-gold text-xs font-bold uppercase tracking-widest block mb-2">The Collection</span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="glass-card rounded-xl p-6 flex flex-col justify-between theme-transition">
                <div>
                    ...
                    <div class="flex items-start justify-between border-b border-gray-500/10 pb-4 mb-4 gap-4">
                        <div>
                            <h3 class="text-lg font-bold tracking-wide text-mosh-gold mb-1">{{ $product->name }}</h3>
                            <span class="text-[11px] font-mono font-medium tracking-wider text-gray-500 uppercase">
                                Unit: {{ $product->sales_unit ?? 'piece' }}
                            </span>
                        </div>
                        <!-- Applied custom theme engine engine hook 'id-price-text' -->
                        <span class="text-base font-mono font-bold text-white id-price-text bg-mosh-gold/10 px-3 py-1 rounded border border-mosh-gold/20 tracking-tight theme-transition">
                    ...
                    <div class="mb-6">
                        <span class="text-[10px] uppercase tracking-widest font-bold text-mosh-gold/80 block mb-2">Composition Base:</span>
                ...
                <div class="pt-4 border-t border-gray-500/10 flex items-center justify-between text-xs">
                    <div class="flex items-center space-x-1.5">
                        <span class="w-2 h-2 rounded-full {{ $product->isLowStock() ? 'bg-amber-500' : 'bg-mosh-accent' }}"></span>
                        <span class="text-gray-500 font-medium">
                            @php
                                // Fallback to 'piece' if sales_unit is empty
                                $unit = $product->sales_unit ?? 'piece';
                            @endphp
                    ...
                    <a href="{{ url('/order?product=' . $product->id) }}" class="bg-mosh-gold hover:bg-opacity-95 text-white font-bold px-4 py-2 rounded text-[11px] uppercase tracking-wider transition">
                        Order Treat
                    </a>
                </div>
            </div>
        @empty
            <div class="col-span-full text-center py-16 border border-dashed border-gray-400/30 rounded-xl glass-card">
                <i class="mdi mdi-cookie-alert text-3xl text-mosh-gold block mb-3"></i>
